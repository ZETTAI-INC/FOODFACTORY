...
export default function QRPage() {
  ...
  return (
    <div className="space-y-6">
          ...
          {products.map((product) => (
            ...
            >
              ...
              <div className="flex gap-2">
                <button className="flex-1 flex items-center justify-center gap-1 text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-800/30 px-3 py-2 rounded-lg transition-colors">
                  <Download size={12} />
                  保存
                </button>
                <button className="flex-1 flex items-center justify-center gap-1 text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-600 px-3 py-2 rounded-lg transition-colors">
                  <Printer size={12} />
                  印刷
                </button>
              </div>
      ...
      {activeTab === "print" && (
        <div>
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-slate-700 rounded-xl p-4 mb-6 flex items-center gap-3 print:hidden">
            <Printer size={20} className="text-amber-600 dark:text-amber-400 shrink-0" />
            <p className="text-sm text-amber-800 dark:text-amber-200">
              印刷用に最適化されたレイアウトです。「印刷する」ボタンを押すか、Ctrl+P で印刷できます。
            </p>
          </div>

          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-8 print:border-none print:p-0 print:rounded-none">
            <div className="text-center mb-6 print:mb-4">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">商品QRコード一覧</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 print:grid-cols-4 print:gap-4">
              {products.map((product) => (
                <div
                  key={product.id}
                  className="flex flex-col items-center p-4 border border-slate-100 dark:border-slate-700 rounded-lg print:border-slate-200 print:rounded-none print:p-3"
                >
                  ...
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100 text-center leading-tight">
                    {product.name}
                  </p>
            ...
            <div className="text-center mt-8 pt-4 border-t border-slate-100 dark:border-slate-700 print:mt-4 print:pt-2">
